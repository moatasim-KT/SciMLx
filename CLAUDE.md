...
- `autorun.py` — subprocess runner. Deduplicates, classifies crashes, **early divergence detection** (kills run if mid-run val > baseline×50 at ≥30% progress), **3-strategy retry chain** (r1=smart-fix, r2=half-model, r3=minimal), **adaptive retry on poor results** (completed val > baseline×3 → `HypothesisEngine.suggest_intervention()` picks a better config and retries once), sentinel kill, `--max-auto-experiments`/`--max-auto-time` guards, git commit on keep. When `--auto` exhausts queue, invokes `agent_loop.py --top 5` and recurses.
