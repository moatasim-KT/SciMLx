...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        queries = self.query_proj(queries).reshape(B, L, H, d).permute(0, 2, 1, 3)
        keys = self.key_proj(keys).reshape(B, S, H, d).permute(0, 2, 1, 3)
        values = self.value_proj(values).reshape(B, S, H, d).permute(0, 2, 1, 3)

        # scores: [B, H, L, S]
        scores = (queries @ keys.permute(0, 1, 3, 2)) * self.scale
        if mask is not None:
            scores = scores + mask

        attn = F.softmax(scores, dim=-1)
        out = (attn @ values).permute(0, 2, 1, 3).reshape(B, L, D)
