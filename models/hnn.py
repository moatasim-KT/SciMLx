...
Adaptations for 1-D PDE operator learning:
  ...
  - HamiltonianNO1d: Hamiltonian Neural Operator for operator learning
    ...
    4. Output: u_pred = q + α*(∂H/∂p) (one "Hamiltonian step" from IC)
...
class HamiltonianNO1d(nn.Module):
    ...
    def __init__(self, n_sensors: int, hidden_dim: int = 64,
                 latent_dim: int = 0, n_layers: int = 4,
                 n_modes: int = 16):
        super().__init__()
        if latent_dim == 0:
    ...
    def __call__(self, u0: mx.array) -> mx.array:
        ...
        # One Hamiltonian step
        dq, _ = self._hamiltonian_step(q, p)
        q_next = q + dq      # [B, latent_dim]
...
class EnergyConservingFNO1d(nn.Module):
    ...
    def __init__(self, n_modes: int, hidden_dim: int, n_layers: int,
        ...
        self.fno = FNO1d(n_modes=n_modes, hidden_dim=hidden_dim,
                         n_layers=n_layers, in_ch=in_ch)
